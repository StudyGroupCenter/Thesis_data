...
class sdCache:
    def __init__(self, c, f, logfile, algn, query_file, k=4, sort_query='sort'):
        self.query_file = query_file  # test set
        self.count = 0  # query count
        self.filename = logfile  # train set
        self.table = {}
        self.trainset = []  # (query,frequency)
        self.sCache = staticCache(int(round(c * f)))
        self.algn=algn
        alg_module = l[algn]
        self.c = c  # cache size
        self.alg = alg_module.alg(int(round(self.c - self.sCache.c)), k=k)
...
#                print "test", i
    ...
    def print_statistics(self):
        print "f:", self.f, "alg:", self.algn, "c:", self.c
...
#pre_handler('training','sort')
size_index = 0
size = [4000000, 6000000]

while size_index < len(size):
        sf = 0
        test = sdCache(size[size_index], sf, 'training', "LIRS", "testing")
        test.fill()
        test.warm_up()
        #test.sCache.show()
        test.cache_test()
